...
setwd(dir = "/Users/Sandra.Gomez/Desktop/Microbiome_analysis/Tar_spot_overtime/Single_reads/R/qiime_results")
...
treatments<-as.vector(unique(dataset$sample.time))
datasetn<-dataset
datasetn[datasetn==0]<-NA

#i = 1

summary(metadata$sample.time)

my_column <- "sample.time"
n1 <- 12
n2 <- 12
n3 <- 12
n4 <- 12
...
#This is filtering, sample.time has three treatments, early, middle and late
sample_time_ASVs<-subset(dataset, get(my_column)==treatments[1])

colnames(sample_time_ASVs[1:10])
head(final_results)
ggplot(sample_time_ASVs, aes(x = ASV106, y = ASV30)) +
  geom_point()
...
write.csv(subset(strong_results_taxa, trt=="Early"), "output/strong-results-taxa-Early.csv", row.names = F)
write.csv(subset(strong_results_taxa, trt=="Middle"), "output/strong-results-taxa-Middle.csv", row.names = F)
write.csv(subset(strong_results_taxa, trt=="Late"), "output/strong-results-taxa-Late.csv", row.names = F)
